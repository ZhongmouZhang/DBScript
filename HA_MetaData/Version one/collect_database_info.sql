...
begin
    ...
    --  and domain='PNG'
	-- where info_update_status like 'Database_INfo'  and is_in_use= 'N'  -- to refresh the server which failed the refresh
	...
	INSERT INTO [CMDB].[dbo].[tbl_db_info]
			 ([instance_info_id],[database_name],[dbid],[owner],[size],[created],[status],[updateability]
			 ,[user_access],[recovery_model],[Version],[compatibility_Level] ,[filename] ,[Status1],[Status2])
	select  @instance_id, database_name,dbid,owner,size, created,status, updateability,
				user_access, recovery_model, version, compatibility_Level,filename,status1, status2
	from #temp
