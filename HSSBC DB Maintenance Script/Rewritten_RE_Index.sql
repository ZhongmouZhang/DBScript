...
@FragThreshold INT  = 30        -- the threshold for rebuild an index
as
...
--print '(2)frag=' + convert(varchar(20),@frag)
			...
			END
		END
		ELSE IF (@frag >= @FragThreshold OR @Allow_pg_locks = 0) 
					AND @LobCount = 0 
					AND @Edition = 'Enterprise Edition'
			-- Re-build the index
	    BEGIN
		...
		BEGIN
			...
			BEGIN
				SET @command = @command + ' PARTITION =' + CAST(@partitionnum AS NVARCHAR(10));
			END
			set @command = @command + ' WITH(ONLINE = OFF, SORT_IN_TEMPDB = ON)';
		END
		ELSE
			set @command = ''
