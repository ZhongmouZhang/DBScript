   if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].hsp_RestoreDB_V2') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].hsp_RestoreDB_V2
GO

CREATE         proc dbo.hsp_RestoreDB_V2
as
...
begin
	...
	set @stt = @stt + ' norecovery'
	-- if (@backuptype!='Full') set @stt = @stt + ', replace'
